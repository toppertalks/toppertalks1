    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <h1 style={{ margin: "0 0 16px", fontSize: 22, fontWeight: 900, color: "#e2e8f0" }}>
                    🔍 Find Toppers
                </h1>
            ...
            <div style={{ padding: "16px 14px" }}>
                ...
                {!query && (
                    <div style={{ animation: "fadeUp 0.3s ease" }}>
                        <p style={{ margin: "0 0 12px", fontSize: 12, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: 0.6 }}>
                            Popular Searches
                        </p>
                        ...
                        {ALL_TOPPERS.filter(t => t.isOnline).slice(0, 3).map(t => (
                            <TopperRow key={t.id} topper={t} onClick={() => router.push(`/topperprofile/${t.id}`)} />
                        ))}
                    </div>
                )}
...
function TopperRow({ topper, onClick }: { topper: typeof ALL_TOPPERS[0]; onClick: () => void }) {
    const tagColor = TAG_COLORS[topper.tag] ?? "#6366f1";
    return (
        <div onClick={onClick} style={{
            display: "flex", alignItems: "center", gap: 12,
            background: "#161b27", border: "1px solid rgba(255,255,255,0.05)",
            borderRadius: 18, padding: "14px", marginBottom: 10, cursor: "pointer",
        ...
        >
            <div style={{ position: "relative", flexShrink: 0 }}>
                <img src={topper.avatarUrl} alt={topper.name} style={{
                    width: 52, height: 52, borderRadius: "50%", objectFit: "cover", objectPosition: "top",
                    border: `2px solid ${tagColor}50`
                }} />
                <div style={{
                    position: "absolute", bottom: 1, right: 1, width: 12, height: 12, borderRadius: "50%",
                    background: topper.isOnline ? "#22c55e" : "#334155", border: "2px solid #161b27"
                }} />
            </div>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                    ...
                    <span style={{
                        background: `rgba(${topper.tag === "IIT" ? "99,102,241" : topper.tag === "NIT" ? "34,197,94" : "14,165,233"},0.12)`,
                        borderRadius: 50, padding: "1px 7px", fontSize: 9, fontWeight: 800, color: tagColor, flexShrink: 0
                    }}>{topper.tag}</span>
                </div>
