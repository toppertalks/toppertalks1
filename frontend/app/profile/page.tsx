...
import {
  Star, Clock, IndianRupee, BookOpen, Settings, ChevronRight,
  Award, Flame, Target, LogOut, GraduationCap
} from "lucide-react";
...
export default function ProfilePage() {
  ...
  const userName = typeof window !== "undefined" ? (localStorage.getItem("tt_user_name") ?? "Student") : "Student";

  useEffect(() => { setLoggedIn(isLoggedIn()); }, []);

  // ── Guest view — shown when not logged in ──
  if (!loggedIn) return (
    <div style={{ minHeight: "100vh", maxWidth: 480, margin: "0 auto", background: "#0d1117", display: "flex", flexDirection: "column", paddingBottom: 72, alignItems: "center", justifyContent: "flex-start" }}>
      {/* Blurred fake header */}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", width: "100%" }}>
        ...
        <p style={{ margin: "0 0 28px", fontSize: 14, color: "#64748b", textAlign: "center", lineHeight: 1.6 }}>
          Sign in to see your sessions, wallet balance, and achievements
        </p>
        ...
        <button onClick={() => setShowLoginModal(true)} style={{
          ...
          cursor: "pointer", boxShadow: "0 6px 24px rgba(99,102,241,0.4)", marginBottom: 10
        }}>Sign In to View Profile</button>
        <p style={{ margin: 0, fontSize: 12, color: "#334155" }}>100% free · Your data stays private</p>
      </div>
    ...
    <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <div style={{
              ...
              background: "#22c55e", border: "2.5px solid #0d1117",
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#e2e8f0", letterSpacing: -0.3 }}>
                Vivek Kumar
              </h1>
              {/* Level badge */}
              <div style={{
            ...
            <p style={{ margin: "3px 0 0", fontSize: 11, color: "#64748b" }}>
              JEE 2025 Aspirant · ID: 166409025
            </p>
            {/* Streak */}
            <div style={{
      ...
      <main style={{ flex: 1, overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", gap: 8, padding: "14px 14px 6px" }}>
          {[
            { icon: "📞", val: "14", label: "Sessions", color: "#818cf8" },
            { icon: "⏱️", val: "6.2 hrs", label: "Study Time", color: "#22c55e" },
            { icon: "⭐", val: "4.8", label: "Avg Rating", color: "#fbbf24" },
          ].map(s => (
        ...
        <div style={{ margin: "6px 14px" }}>
          <div style={{
            ...
            <div>
              <p style={{
                margin: 0, fontSize: 11, color: "#94a3b8", textTransform: "uppercase",
                fontWeight: 600, letterSpacing: 0.8
              }}>Wallet Balance</p>
              <p style={{ margin: "4px 0 0", fontSize: 26, fontWeight: 900, color: "#fff", lineHeight: 1 }}>
                ₹120
              </p>
              <p style={{ margin: "4px 0 0", fontSize: 10, color: "#818cf8" }}>
                ≈ 12 min with any IIT topper
              </p>
            </div>
            <button style={{
              background: "linear-gradient(135deg,#6366f1,#8b5cf6)",
              ...
              cursor: "pointer", boxShadow: "0 4px 18px rgba(99,102,241,0.5)"
            }}>
              + Add Money
            </button>
          </div>
        ...
        <div style={{ padding: "14px 14px 6px" }}>
          <p style={{
            margin: "0 0 8px", fontSize: 13, fontWeight: 700, color: "#94a3b8",
        ...
        <div style={{ padding: "10px 14px 0" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
            {RECENT_SESSIONS.map((s, i) => (
              <div key={i} style={{
                background: "#161b27", borderRadius: 14, padding: "12px 14px",
                border: "1px solid rgba(255,255,255,0.06)",
                display: "flex", alignItems: "center", justifyContent: "space-between"
              }}>
                <div>
                ...
                <div style={{ display: "flex", gap: 1 }}>
                  {[1, 2, 3, 4, 5].map(n => (
                    <Star key={n} size={10} fill="#fbbf24" color="#fbbf24" />
                  ))}
                </div>
              </div>
            ))}
        ...
        <div style={{ padding: "14px 14px 8px", display: "flex", flexDirection: "column", gap: 6 }}>
          <p style={{
            margin: "0 0 4px", fontSize: 13, fontWeight: 700, color: "#94a3b8",
            textTransform: "uppercase", letterSpacing: 0.8
          }}>Account</p>
          {MENU.map((item) => {
            ...
            return (
              <button key={item.label}
                ...
                onMouseEnter={e => { if (href) (e.currentTarget as HTMLButtonElement).style.background = "#1e2535"; }}
                onMouseLeave={e => { if (href) (e.currentTarget as HTMLButtonElement).style.background = "#161b27"; }}>
                <div style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ margin: "1px 0 0", fontSize: 11, color: "#475569" }}>{item.sub}</p>
                </div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            {["✅ IIT/NEET Verified", "💰 Avg ₹18K/month", "⏱️ Work Anytime"].map(f => (
              <span key={f} style={{
                ...
                borderRadius: 8, padding: "3px 8px", fontSize: 9, fontWeight: 600, color: "#94a3b8"
              }}>{f}</span>
            ))}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => router.push("/become-mentor")} style={{
              flex: 1, padding: "10px", borderRadius: 12,
              background: "linear-gradient(135deg,#6366f1,#8b5cf6)",
              border: "none", color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer",
              boxShadow: "0 2px 12px rgba(99,102,241,0.4)"
            }}>Apply as Mentor →</button>
            <button onClick={() => router.push("/topper")} style={{
        ...
        {/* App version */}
        <p style={{ textAlign: "center", fontSize: 10, color: "#1e293b", padding: "10px 0 4px", fontWeight: 600 }}>
          TopperTalks v1.0.0 · Made for JEE/NEET Warriors 🎯
        </p>
      </main>
