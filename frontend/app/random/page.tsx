...
const TAG_COLORS: Record<string, string> = {
  IIT: "#6366f1", AIIMS: "#0ea5e9", NIT: "#10b981"
};

type Phase = "idle" | "spinning" | "matched" | "calling";
type HistoryItem = (typeof TOPPERS)[number] & { calledAt: string };

/* slot-machine deceleration delays (ms) */
...
export default function RandomPage() {
  ...
  const [matchedTopper, setMatchedTopper] = useState<typeof TOPPERS[number] | null>(null);
  ...
  const startSpin = useCallback(() => {
    ...
    function tick() {
      ...
      if (next < SPIN_DELAYS.length) {
        setTimeout(tick, SPIN_DELAYS[next]);
      } else {
        /* pick winner */
  ...
  /* ── Continue → require auth, then request mic/cam, then go to calling phase ── */
  const handleContinue = useCallback(async () => {
    ...
    try {
      ...
      setPhase("calling");
      setTimeout(() => router.push(`/messages/${matchedTopper.id}`), 3000);
    } catch {
      setPermDenied(true);
    }
  }, [matchedTopper, router]);
  ...
  return (
    <div style={{
      ...
      position: "relative", overflow: "hidden", paddingBottom: 72
    }}>

      {/* ── Stars ── */}
      <style>{`
        @keyframes twinkle{0%,100%{opacity:.15;transform:scale(1)}50%{opacity:.9;transform:scale(1.5)}}
        @keyframes matchReveal{0%{transform:scale(0.85);opacity:0}60%{transform:scale(1.04)}100%{transform:scale(1);opacity:1}}
        @keyframes pulse{0%,100%{box-shadow:0 0 0 0 rgba(34,197,94,.6)}50%{box-shadow:0 0 0 14px rgba(34,197,94,0)}}
        @keyframes spin{to{transform:rotate(360deg)}}
        @keyframes blink{0%,100%{opacity:1}50%{opacity:0.3}}
        @keyframes slideUp{from{transform:translateY(100%)}to{transform:translateY(0)}}
        @keyframes callRing{0%,100%{transform:scale(1)}50%{transform:scale(1.08)}}
      `}</style>
      {[5, 14, 28, 42, 60, 75, 88, 96, 3, 18, 35, 52, 68, 82, 93, 9, 24, 47, 63, 79, 91, 6, 33, 58, 84].map((l, i) => (
        <div key={i} aria-hidden style={{
          position: "absolute", left: `${l}%`, top: `${8 + i * 3.5}%`,
          width: i % 3 === 0 ? 1.8 : 1.2, height: i % 3 === 0 ? 1.8 : 1.2,
          borderRadius: "50%", background: "#fff",
          animation: `twinkle ${2 + i % 4 * 0.4}s ${i * 0.3}s ease-in-out infinite`,
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <span style={{ fontFamily: "system-ui", fontWeight: 800, fontSize: 17, color: "#fff" }}>
              Topper<span style={{
        ...
        <div style={{
          ...
          <div style={{ position: "relative", width: 260, height: 260 }}>
            {TOPPERS.slice(0, 4).map((t, i) => {
              ...
              return (
                <div key={t.id} style={{
                  ...
                  <img src={t.avatarUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "top" }} />
                </div>
              );
            })}
            {/* Center pulse */}
            <div style={{
              ...
              animation: "pulse 2s infinite", zIndex: 2, fontSize: 28
            }}>🎲</div>
          </div>

          <div style={{ textAlign: "center" }}>
            <h1 style={{ margin: 0, fontSize: 22, fontWeight: 900, color: "#e2e8f0", lineHeight: 1.2 }}>
              Get Matched with a<br />
              <span style={{
                background: "linear-gradient(90deg,#818cf8,#c084fc)",
                WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent"
              }}>
            ...
            <p style={{ margin: "10px 0 0", fontSize: 13, color: "#64748b", lineHeight: 1.6 }}>
              Our system instantly matches you with the<br />best available mentor for your subject.
            </p>
          </div>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: "#4ade80" }}>4,731 online</span>
            </div>
          ...
          <p style={{ margin: 0, fontSize: 11, color: "#334155", textAlign: "center" }}>
            ₹50 for your first session · Then ₹10–₹50/min<br />
            No booking needed · Cancel anytime
          </p>
        </div>
      )}

      {/* ══════════════════════════════════════
          PHASE: SPINNING (slot machine)
      ══════════════════════════════════════ */}
      {phase === "spinning" && (
        <div style={{
          position: "relative", zIndex: 5,
          ...
          <div style={{ textAlign: "center" }}>
            <p style={{
              margin: 0, fontSize: 14, color: "#818cf8", fontWeight: 700,
              letterSpacing: 1.5, textTransform: "uppercase"
            }}>
              Matching you with
            </p>
            <h2 style={{ margin: "4px 0 0", fontSize: 20, fontWeight: 800, color: "#e2e8f0" }}>
              the perfect topper...
            </h2>
          </div>
          ...
          <div style={{
            ...
            boxShadow: "0 0 32px rgba(99,102,241,0.6), 0 0 64px rgba(99,102,241,0.3)"
          }}>
            <img
          ...
          <div style={{
            ...
            <p style={{
              margin: 0, fontSize: 13, fontWeight: 700, color: "#a5b4fc",
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {[0, 1, 2].map(i => (
              <div key={i} style={{
                ...
                background: "#6366f1", opacity: 0.4 + (i * 0.3),
                animation: `blink 0.8s ${i * 0.27}s ease-in-out infinite`
              }} />
            ))}
          </div>
        </div>
      ...
      {phase === "matched" && matchedTopper && (
        <div style={{
          position: "relative", zIndex: 5,
          flex: 1, display: "flex", flexDirection: "column",
          alignItems: "center", padding: "20px 20px 0", gap: 16
        }}>
          {/* Match banner */}
          <div style={{
            background: "linear-gradient(90deg,rgba(34,197,94,0.12),rgba(99,102,241,0.12))",
          ...
          <div style={{
            ...
            animation: "matchReveal 0.6s ease-out forwards",
            boxShadow: "0 0 0 3px #22c55e, 0 0 40px rgba(34,197,94,0.5), 0 8px 40px rgba(0,0,0,0.5)"
          }}>
            <img src={matchedTopper.avatarUrl} alt={matchedTopper.name} style={{
              width: "100%", height: "100%", objectFit: "cover", objectPosition: "top"
            }} />
            <div style={{
              position: "absolute", inset: 0,
              background: "linear-gradient(to top,rgba(13,17,23,0.95) 0%,rgba(13,17,23,0.3) 55%,transparent 70%)"
            }} />
            {/* Tag */}
            <div style={{
              position: "absolute", top: 14, right: 14,
              background: TAG_COLORS[matchedTopper.tag] || "#6366f1",
              borderRadius: 10, padding: "3px 10px",
              fontSize: 10, fontWeight: 800, color: "#fff"
            }}>🏛️ {matchedTopper.tag}</div>
            {/* Online */}
            <div style={{
            ...
            <div style={{ position: "absolute", bottom: 16, left: 16, right: 16 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 6 }}>
                {[1, 2, 3, 4, 5].map(n => (
                  <Star key={n} size={11} fill="#fbbf24" color="#fbbf24" />
                ))}
          ...
          {/* Action buttons */}
          <div style={{
            display: "flex", gap: 10, width: "100%", maxWidth: 360, paddingTop: 4
          }}>
            ...
            {permDenied && (
              <div style={{
                ...
                <p style={{ margin: "4px 0 0", fontSize: 11, color: "#94a3b8", lineHeight: 1.5 }}>
                  Please allow camera and microphone in your browser settings, then try again.
                </p>
                <button onClick={() => setPermDenied(false)} style={{
                  marginTop: 8, padding: "5px 12px", borderRadius: 8, fontSize: 11, fontWeight: 700,
      ...
      {phase === "calling" && matchedTopper && (
        <div style={{
          ...
          <div style={{
            ...
            boxShadow: "0 0 0 4px #22c55e, 0 0 40px rgba(34,197,94,0.6)",
            animation: "callRing 1s ease-in-out infinite"
          }}>
            <img src={matchedTopper.avatarUrl} alt="" style={{
              width: "100%", height: "100%", objectFit: "cover", objectPosition: "top"
            }} />
          </div>
          <div>
            <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#e2e8f0" }}>
              Connecting to {matchedTopper.name}...
            </h2>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            <div style={{
              width: 8, height: 8, borderRadius: "50%", background: "#22c55e",
              animation: "blink 0.7s 0s ease-in-out infinite"
            }} />
            <div style={{
              width: 8, height: 8, borderRadius: "50%", background: "#22c55e",
              animation: "blink 0.7s 0.25s ease-in-out infinite"
            }} />
            <div style={{
              width: 8, height: 8, borderRadius: "50%", background: "#22c55e",
              animation: "blink 0.7s 0.5s ease-in-out infinite"
            }} />
          </div>
              ...
              {history.length === 0 ? (
                ...
                <div key={i} style={{
                  ...
                  <img src={item.avatarUrl} alt="" style={{
                    width: 46, height: 46, borderRadius: 12, objectFit: "cover", objectPosition: "top"
                  }} />
                  <div style={{ flex: 1 }}>
                  ...
                  <div style={{
                    ...
                    fontSize: 10, fontWeight: 700, color: TAG_COLORS[item.tag] || "#6366f1"
                  }}>{item.tag}</div>
                </div>
              ))}
            </div>
      ...
      {/* ── Login Modal (only shown when needed) ── */}
      {showLoginModal && (
        <LoginModal
          reason="connect with a mentor"
          returnTo={matchedTopper ? `/messages/${matchedTopper.id}` : "/random"}
