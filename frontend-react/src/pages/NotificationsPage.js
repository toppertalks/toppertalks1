      ...
      <header style={{ background: "linear-gradient(180deg,#13192b,#0d1117)", borderBottom: "1px solid rgba(99,102,241,0.15)", padding: "14px 16px", position: "sticky", top: 0, zIndex: 30 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {unreadCount > 0 && <button onClick={markAllRead} style={{ padding: "6px 12px", borderRadius: 10, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", color: "#818cf8", fontSize: 11, fontWeight: 700, cursor: "pointer" }}><CheckCircle size={12} style={{ display: "inline", marginRight: 4 }} />Mark all read</button>}
            {notifs.length > 0 && <button onClick={clearAll} style={{ padding: "6px 10px", borderRadius: 10, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.15)", color: "#f87171", fontSize: 11, fontWeight: 700, cursor: "pointer" }}><Trash2 size={12} style={{ display: "inline" }} /></button>}
          </div>
        </div>
        <div style={{ display: "flex", gap: 6, marginTop: 12, overflowX: "auto", paddingBottom: 2 }}>
          {FILTERS.map(f => (
            <button key={f.key} onClick={() => setActiveFilter(f.key)} style={{ padding: "5px 12px", borderRadius: 50, fontSize: 11, fontWeight: 700, background: activeFilter === f.key ? "rgba(99,102,241,0.2)" : "rgba(255,255,255,0.04)", border: `1px solid ${activeFilter === f.key ? "rgba(99,102,241,0.4)" : "rgba(255,255,255,0.08)"}`, color: activeFilter === f.key ? "#818cf8" : "#475569", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>{f.label}</button>
          ))}
        </div>
      </header>
            ...
            return (
              <div key={group} style={{ marginBottom: 20 }}>
                ...
                {items.map(notif => {
                  ...
                  return (
                    <div key={notif.id} style={{ display: "flex", alignItems: "flex-start", gap: 12, background: notif.read ? "#161b27" : "rgba(99,102,241,0.08)", border: `1px solid ${notif.read ? "rgba(255,255,255,0.04)" : "rgba(99,102,241,0.2)"}`, borderRadius: 18, padding: "14px", marginBottom: 8, position: "relative", animation: "fadeUp 0.25s ease" }}>
                      {notif.avatarUrl ? (
                        ...
                        <div style={{ width: 44, height: 44, borderRadius: 14, background: meta.bg, display: "flex", alignItems: "center", justifyContent: "center", color: meta.color, flexShrink: 0 }}>{meta.icon}</div>
                      )}
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          ...
                          <button onClick={() => dismiss(notif.id)} style={{ background: "none", border: "none", color: "#334155", cursor: "pointer", padding: "2px 4px" }}><Trash2 size={12} /></button>
                        </div>
                      </div>
                    </div>
                  );
