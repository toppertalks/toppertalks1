...
export default function ChatPage() {
  ...
  const rtcCredsRef = useRef(null); // {token, url} from /api/sessions/start
  ...
  useEffect(() => {
    ...
    const creds = rtcCredsRef.current;
    if (!creds?.token) return; // start hasn't returned yet
    let cancelled = false;
    startZegoCall({
      token: creds.token,
      url: creds.url,
      ...
      .catch(err => console.warn("LiveKit call error:", err));
    return () => { cancelled = true; };
  }, [showCalling, callType]);
  ...
  const requestPermAndCall = useCallback(async (type) => {
    ...
    try {
      ...
      rtcCredsRef.current = { token: sessionResult.livekitToken, url: sessionResult.livekitUrl };
