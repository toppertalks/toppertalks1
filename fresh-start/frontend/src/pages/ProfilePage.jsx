...
export default function ProfilePage() {
  ...
  // Display values derived from the signed-in user.
  const displayName = user?.full_name || user?.username || (user?.email ? user.email.split("@")[0] : "Guest");
  const displayEmail = user?.email || "";
  const displayInitial = (displayName || "?").trim().charAt(0).toUpperCase();
  const examMode = user?.exam_mode || "JEE";
  const userIdShort = user?.id ? String(user.id).replace(/-/g, "").slice(0, 9) : "";
  ...
  const menuItems = [
    ...
    { icon: LogOut, label: "Sign Out", sub: `You are signed in as ${displayName}`, color: "#ef4444", onClick: handleSignOut },
  ];
      ...
      <div style={{ background: "linear-gradient(180deg,#13192b 0%,#0d1117 100%)", borderBottom: "1px solid rgba(99,102,241,0.15)", padding: "20px 18px 18px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ position: "relative" }}>
            <div style={{ width: 64, height: 64, borderRadius: 18, background: "linear-gradient(135deg,#6366f1,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, fontWeight: 800, color: "#fff", boxShadow: "0 4px 20px rgba(99,102,241,0.5)" }}>{displayInitial}</div>
            <div className="online-dot" style={{ position: "absolute", bottom: -2, right: -2, width: 14, height: 14, border: "2.5px solid #0d1117" }} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#e2e8f0", letterSpacing: -0.3 }}>{displayName}</h1>
            ...
            {displayEmail && (
              <p style={{ margin: "3px 0 0", fontSize: 11, color: "#94a3b8" }}>{displayEmail}</p>
            )}
            <p style={{ margin: "3px 0 0", fontSize: 11, color: "#64748b" }}>{examMode} 2025 Aspirant{userIdShort ? ` · ID: ${userIdShort}` : ""}</p>
