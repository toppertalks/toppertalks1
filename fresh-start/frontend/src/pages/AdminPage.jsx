...
export default function AdminPage() {
  ...
  const approveApp = (id) => setApplications(a => a.filter(x => x.id !== id));
  const rejectApp = (id) => setApplications(a => a.filter(x => x.id !== id));
  const resolveReport = (id) => setReports(r => r.filter(x => x.id !== id));
  const approvePayout = (id) => setPayouts(p => p.filter(x => x.id !== id));
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080c14", fontFamily: "'Inter',sans-serif" }}>
      {/* TOP BAR */}
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", background: "rgba(13,19,35,0.97)", borderBottom: "1px solid rgba(99,102,241,0.15)", position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(16px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: "linear-gradient(135deg,#6366f1,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 16px rgba(99,102,241,0.4)" }}><Shield size={18} color="#fff" /></div>
          <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ position: "relative" }}>
            ...
            <div style={{ position: "absolute", top: -4, right: -4, width: 14, height: 14, borderRadius: "50%", background: "#ef4444", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 800, color: "#fff" }}>{OVERVIEW.openReports}</div>
          </div>
      ...
      <div style={{ display: "flex", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <aside style={{ width: 200, flexShrink: 0, padding: "20px 12px", height: "calc(100vh - 60px)", position: "sticky", top: 60, overflowY: "auto", borderRight: "1px solid rgba(255,255,255,0.05)" }}>
          {NAV.map(item => (
            <button key={item.key} onClick={() => setTab(item.key)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", borderRadius: 12, marginBottom: 4, cursor: "pointer", background: tab === item.key ? "rgba(99,102,241,0.15)" : "transparent", border: `1px solid ${tab === item.key ? "rgba(99,102,241,0.3)" : "transparent"}`, color: tab === item.key ? "#818cf8" : "#475569", textAlign: "left", transition: "all 0.15s" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 700 }}>{item.icon}{item.label}</div>
        ...
        <main style={{ flex: 1, padding: "20px 24px", animation: "fadeUp 0.25s ease", overflowX: "auto" }}>
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12, marginBottom: 24 }}>
                {[
                  ...
                  <div key={k.label} style={{ background: "#161b27", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 18, padding: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <span style={{ fontSize: 24 }}>{k.icon}</span>
                      {k.pulse && <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#22c55e", animation: "pulse 1.5s ease-in-out infinite" }} />}
                    </div>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {[
                  { label: `Review ${applications.filter(a => a.status === "pending").length} Applications`, color: "#6366f1", rgb: "99,102,241", onClick: () => setTab("applications") },
                  { label: `Resolve ${reports.filter(r => r.status === "open").length} Reports`, color: "#ef4444", rgb: "239,68,68", onClick: () => setTab("reports") },
                  { label: `Approve ${payouts.length} Payouts`, color: "#22c55e", rgb: "34,197,94", onClick: () => setTab("payouts") },
                ].map(a => (
                  <button key={a.label} onClick={a.onClick} style={{ padding: "10px 18px", borderRadius: 12, cursor: "pointer", background: `rgba(${a.rgb},0.12)`, border: `1px solid ${a.color}40`, color: a.color, fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <div>
              ...
              {applications.length === 0 ? (
                ...
                applications.map(app => (
                  <div key={app.id} style={{ background: "#161b27", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 20, padding: 20, marginBottom: 12 }}>
                    <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: "flex", gap: 10 }}>
                          <button style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 10, background: "rgba(14,165,233,0.1)", border: "1px solid rgba(14,165,233,0.25)", color: "#38bdf8", fontSize: 12, fontWeight: 700, cursor: "pointer" }}><Eye size={14} /> View Docs</button>
                          <button onClick={() => approveApp(app.id)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 10, background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.3)", color: "#4ade80", fontSize: 12, fontWeight: 700, cursor: "pointer" }}><CheckCircle size={14} /> Approve</button>
                          <button onClick={() => rejectApp(app.id)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 10, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", color: "#f87171", fontSize: 12, fontWeight: 700, cursor: "pointer" }}><XCircle size={14} /> Reject</button>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {tab === "reports" && (
            <div>
              ...
              {reports.length === 0 ? (
                ...
                reports.map(r => (
                  <div key={r.id} style={{ background: "#161b27", border: `1px solid ${r.status === "open" ? "rgba(239,68,68,0.2)" : "rgba(251,191,36,0.2)"}`, borderRadius: 20, padding: 18, marginBottom: 12 }}>
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                      <button onClick={() => resolveReport(r.id)} style={{ flex: 1, padding: 10, borderRadius: 10, background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.25)", color: "#4ade80", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>✅ Release Payment (Legit)</button>
                      <button onClick={() => resolveReport(r.id)} style={{ flex: 1, padding: 10, borderRadius: 10, background: "rgba(239,68,68,0.09)", border: "1px solid rgba(239,68,68,0.2)", color: "#f87171", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>🚫 Refund Student (Valid)</button>
                    </div>
                  </div>
                ))
              )}
          ...
          {tab === "payouts" && (
            <div>
              ...
              {payouts.length === 0 ? (
                ...
                payouts.map(p => (
                  <div key={p.id} style={{ background: "#161b27", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 18, padding: 18, marginBottom: 12 }}>
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                      <button onClick={() => approvePayout(p.id)} style={{ flex: 1, padding: 10, borderRadius: 10, background: "linear-gradient(135deg,#22c55e,#16a34a)", border: "none", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>✅ Approve & Transfer</button>
                      <button onClick={() => approvePayout(p.id)} style={{ width: 44, height: 44, borderRadius: 10, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", color: "#f87171", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><XCircle size={18} /></button>
                    </div>
                  </div>
                ))
              )}
          ...
          {tab === "toppers" && (
            <div>
              ...
              {TOP_TOPPERS.map((t, i) => (
                <div key={t.name} style={{ display: "flex", alignItems: "center", gap: 14, background: "#161b27", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 20, padding: 18, marginBottom: 12 }}>
                  <div style={{ fontSize: 22, fontWeight: 900, color: i === 0 ? "#fbbf24" : i === 1 ? "#94a3b8" : "#cd7c32", width: 28, textAlign: "center" }}>#{i + 1}</div>
                  ...
                  <button style={{ padding: "8px 14px", borderRadius: 10, background: "rgba(99,102,241,0.08)", border: "1px solid rgba(99,102,241,0.2)", color: "#818cf8", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>View Profile</button>
                </div>
              ))}
            </div>
          )}
