...
import { updateTopperStatus } from "../lib/api-client";
import { getLocalUser } from "../lib/auth-guard";
import { startZegoCall, stopZegoCall } from "../lib/zego";

const TOPPER = {
  name: "Ananya Sharma", college: "IIT Bombay", tag: "IIT",
  subjects: ["Physics", "Maths", "Mechanics"], rate: 10, topperCut: 6, platformCut: 4, minCharge: 50,
  rating: 4.9, totalSessions: 312,
  avatarUrl: "https://images.pexels.com/photos/1181686/pexels-photo-1181686.jpeg",
  todayEarnings: 1260, todaySessions: 7, weeklyEarnings: 8400, pendingPayout: 5040, walletBalance: 5040
};
...
const INCOMING_STUDENT = {
  id: "1", name: "Rahul Sharma", class: "Class 12 · JEE 2026", subject: "Rotational Dynamics",
  avatar: "https://images.pexels.com/photos/614810/pexels-photo-614810.jpeg", callType: "video"
};

function TopperNav({ active, onChange }) {
  const items = [
  ...
  return (
    <nav style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, zIndex: 40, background: "rgba(13,17,23,0.95)", backdropFilter: "blur(20px)", borderTop: "1px solid rgba(99,102,241,0.15)" }}>
      <div style={{ display: "flex" }}>
        {items.map(item => {
          const on = item.key === active;
          return (
            <button key={item.key} onClick={() => onChange(item.key)} style={{ flex: 1, padding: "10px 0 12px", background: "none", border: "none", color: on ? "#818cf8" : "#334155", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
              {on && <div style={{ width: 24, height: 2.5, background: "linear-gradient(90deg,#6366f1,#8b5cf6)", borderRadius: 2, marginBottom: 2 }} />}
              {item.icon}
...
export default function TopperDashboardPage() {
  ...
  const zegoTopperSessionRef = useRef(null);
  const topperRemoteAudioRef = useRef(null);
  const topperLocalVideoRef = useRef(null);
  const topperRemoteVideoRef = useRef(null);
  ...
  const acceptCall = () => {
    ...
    startZegoCall({
      conversationId: INCOMING_STUDENT.id, role: "topper", isVideo: false,
      remoteAudioEl: topperRemoteAudioRef.current, remoteVideoEl: topperRemoteVideoRef.current,
      onRemoteUserJoined: () => setStudentJoined(true),
    }).then(session => { zegoTopperSessionRef.current = session; })
  ...
  const endTopperCall = () => {
    ...
    if (zegoTopperSessionRef.current) { stopZegoCall(zegoTopperSessionRef.current); zegoTopperSessionRef.current = null; }
  ...
  return (
    <div style={{ minHeight: "100vh", maxWidth: 480, margin: "0 auto", background: "#0d1117", display: "flex", flexDirection: "column", paddingBottom: 72 }}>
      {/* HEADER */}
      <header style={{ background: "linear-gradient(180deg,#13192b,#0d1117)", borderBottom: "1px solid rgba(99,102,241,0.15)", padding: "14px 16px", position: "sticky", top: 0, zIndex: 30 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ position: "relative", flexShrink: 0 }}>
            <img src={TOPPER.avatarUrl} alt="" style={{ width: 44, height: 44, borderRadius: "50%", objectFit: "cover", objectPosition: "top", border: `2px solid ${isOnline ? "#22c55e" : "rgba(255,255,255,0.15)"}` }} />
            <div style={{ position: "absolute", bottom: 0, right: 0, width: 13, height: 13, borderRadius: "50%", background: isOnline ? "#22c55e" : "#334155", border: "2px solid #0d1117", boxShadow: isOnline ? "0 0 8px #22c55e" : "none" }} />
          </div>
          <div style={{ flex: 1 }}>
          ...
          <button style={{ width: 38, height: 38, borderRadius: "50%", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", color: "#475569", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}><Bell size={17} /></button>
        </div>
      </header>
      ...
      {activeTab === "dashboard" && (
        <main style={{ flex: 1, overflowY: "auto", padding: "16px 14px 0" }}>
          {/* BIG ONLINE TOGGLE */}
          <div style={{ borderRadius: 24, padding: "24px 20px", marginBottom: 16, textAlign: "center", background: isOnline ? "linear-gradient(135deg,rgba(22,163,74,0.15),rgba(34,197,94,0.08))" : "linear-gradient(135deg,#161b27,#13192b)", border: `1.5px solid ${isOnline ? "rgba(34,197,94,0.4)" : "rgba(255,255,255,0.06)"}`, boxShadow: isOnline ? "0 0 40px rgba(34,197,94,0.15)" : "none", transition: "all 0.4s ease" }}>
            ...
            <button onClick={async () => {
              ...
              try {
                ...
                if (user?.uid) await updateTopperStatus(user.uid, newState);
              } catch (err) { console.warn("Failed to update online status:", err); setIsOnline(!newState); }
            }} style={{ width: 72, height: 72, borderRadius: "50%", cursor: "pointer", background: isOnline ? "linear-gradient(135deg,#22c55e,#16a34a)" : "rgba(255,255,255,0.08)", border: isOnline ? "none" : "1.5px solid rgba(255,255,255,0.12)", boxShadow: isOnline ? "0 0 30px rgba(34,197,94,0.5)" : "none", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto", transition: "all 0.3s ease", animation: isOnline ? "callPulse 2s ease-in-out infinite" : "none" }}>
          ...
          {callAccepted && (
            <div style={{ borderRadius: 20, padding: 16, marginBottom: 16, background: "linear-gradient(135deg,rgba(99,102,241,0.2),rgba(139,92,246,0.15))", border: "1.5px solid rgba(99,102,241,0.4)" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <button onClick={() => setCallAccepted(false)} style={{ width: 48, height: 48, borderRadius: "50%", background: "linear-gradient(135deg,#ef4444,#dc2626)", border: "none", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 16px rgba(239,68,68,0.5)" }}><PhoneOff size={20} /></button>
              </div>
            </div>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
            {[
              ...
              <div key={s.label} style={{ background: "#161b27", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 18, padding: 14 }}>
          ...
          {/* PAYOUT CARD */}
          <div style={{ borderRadius: 20, padding: "16px 18px", marginBottom: 12, background: "linear-gradient(135deg,#1e1b4b,#312e81)", border: "1px solid rgba(99,102,241,0.3)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
            ...
            <button onClick={() => setShowPayout(true)} style={{ padding: "10px 18px", borderRadius: 12, background: "linear-gradient(135deg,#6366f1,#8b5cf6)", border: "none", color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer", boxShadow: "0 2px 12px rgba(99,102,241,0.4)" }}>Withdraw</button>
          </div>

          {/* RECENT SESSIONS */}
          <p style={{ margin: "8px 0 10px", fontSize: 12, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: 0.8 }}>Recent Sessions</p>
          {RECENT_SESSIONS.map(s => (
            <div key={s.id} style={{ background: "#161b27", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 16, padding: 14, marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 42, height: 42, borderRadius: "50%", flexShrink: 0, background: "rgba(99,102,241,0.15)", border: "1px solid rgba(99,102,241,0.25)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <main style={{ flex: 1, padding: "16px 14px" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {[{ l: "Total", v: TOPPER.totalSessions }, { l: "This Month", v: 68 }, { l: "Avg Duration", v: "24m" }].map(x => (
              <div key={x.l} style={{ flex: 1, background: "#161b27", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14, padding: "10px 8px", textAlign: "center" }}>
          ...
          {[...RECENT_SESSIONS, ...RECENT_SESSIONS].map((s, i) => (
            <div key={i} style={{ background: "#161b27", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 16, padding: 14, marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 42, height: 42, borderRadius: "50%", flexShrink: 0, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}><User size={18} color="#818cf8" /></div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: "#e2e8f0" }}>{s.student}</p>
                <p style={{ margin: "2px 0 0", fontSize: 11, color: "#64748b" }}><BookOpen size={10} style={{ display: "inline", marginRight: 3 }} />{s.subject}</p>
                <p style={{ margin: "2px 0 0", fontSize: 10, color: "#334155" }}><Clock size={9} style={{ display: "inline", marginRight: 3 }} />{s.duration} min · {s.time}</p>
              </div>
      ...
      {activeTab === "earnings" && (
        <main style={{ flex: 1, padding: "16px 14px" }}>
          ...
          <div style={{ borderRadius: 24, padding: 24, marginBottom: 16, background: "linear-gradient(135deg,#1e1b4b,#312e81)", border: "1px solid rgba(99,102,241,0.3)" }}>
            <p style={{ margin: "0 0 4px", fontSize: 12, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 1 }}>Available Balance</p>
            <p style={{ margin: "0 0 4px", fontSize: 36, fontWeight: 900, color: "#e2e8f0" }}>₹{TOPPER.pendingPayout.toLocaleString()}</p>
            <p style={{ margin: "0 0 16px", fontSize: 11, color: "#64748b" }}>Earned from {TOPPER.totalSessions} sessions</p>
            <button onClick={() => setShowPayout(true)} style={{ width: "100%", padding: 13, borderRadius: 14, background: "linear-gradient(135deg,#6366f1,#8b5cf6)", border: "none", color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", boxShadow: "0 4px 20px rgba(99,102,241,0.4)" }}>💸 Withdraw to UPI / Bank</button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
            {[
              ...
              <div key={x.p} style={{ background: "#161b27", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 18, padding: 14 }}>
          ...
          {[
            ...
            <div key={i} style={{ background: "#161b27", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14, padding: 14, marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
              ...
              <span style={{ background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.25)", borderRadius: 8, padding: "3px 10px", fontSize: 11, fontWeight: 700, color: "#4ade80" }}>✓ {p.status}</span>
            </div>
          ))}
        </main>
      )}
      ...
      {activeTab === "settings" && (
        <main style={{ flex: 1, padding: "16px 14px" }}>
          ...
          <div style={{ background: "#161b27", borderRadius: 20, padding: 16, marginBottom: 12, border: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 4, marginTop: 4 }}>
                {TOPPER.subjects.slice(0, 3).map(s => (
                  <span key={s} style={{ background: "rgba(99,102,241,0.12)", border: "1px solid rgba(99,102,241,0.25)", borderRadius: 6, padding: "1px 6px", fontSize: 9, fontWeight: 700, color: "#818cf8" }}>{s}</span>
                ))}
              </div>
            </div>
            <button style={{ padding: "7px 14px", borderRadius: 10, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.25)", color: "#818cf8", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Edit</button>
          </div>
          {/* Rate card */}
          <div style={{ background: "#161b27", borderRadius: 20, padding: 16, marginBottom: 12, border: "1px solid rgba(255,255,255,0.05)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              ...
              <div style={{ background: "linear-gradient(135deg,#6366f1,#8b5cf6)", borderRadius: 12, padding: "6px 14px" }}>
                <p style={{ margin: 0, fontSize: 16, fontWeight: 900, color: "#fff" }}>₹10/min</p>
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <div style={{ background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 10, padding: "8px 12px" }}>
              ...
              <div style={{ background: "rgba(99,102,241,0.08)", border: "1px solid rgba(99,102,241,0.2)", borderRadius: 10, padding: "8px 12px" }}>
            ...
            <p style={{ margin: "10px 0 0", fontSize: 11, color: "#475569", lineHeight: 1.6 }}>Custom rates will be unlocked as the platform grows. You'll be notified first! 🎉</p>
          </div>
          {/* Menu items */}
          {[
            ...
            <button key={item.label} style={{ width: "100%", display: "flex", alignItems: "center", gap: 14, padding: "14px 16px", marginBottom: 8, borderRadius: 16, background: "#161b27", border: "1px solid rgba(255,255,255,0.05)", cursor: "pointer", textAlign: "left" }}>
              <div style={{ width: 40, height: 40, borderRadius: 12, flexShrink: 0, background: "rgba(99,102,241,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>{item.icon}</div>
              <div style={{ flex: 1 }}>
      ...
      {showIncoming && (
        <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.92)", backdropFilter: "blur(20px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", animation: "slideSheet 0.3s ease", gap: 0 }}>
          <p style={{ position: "absolute", top: 60, right: 24, fontSize: 18, fontWeight: 900, color: "#64748b" }}>{ringCount}s</p>
          <p style={{ margin: "0 0 30px", fontSize: 13, color: "#64748b", textTransform: "uppercase", letterSpacing: 2, fontWeight: 700 }}>Incoming {INCOMING_STUDENT.callType === "video" ? "Video" : "Voice"} Call</p>
          <div style={{ width: 110, height: 110, borderRadius: "50%", overflow: "hidden", border: "3px solid #22c55e", marginBottom: 20, animation: "callRing 1.5s ease-in-out infinite" }}>
          ...
          <div style={{ background: "rgba(99,102,241,0.15)", border: "1px solid rgba(99,102,241,0.3)", borderRadius: 10, padding: "5px 14px", marginBottom: 32 }}>
            <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: "#818cf8" }}>📖 Needs help with: {INCOMING_STUDENT.subject}</p>
          </div>
          <p style={{ margin: "0 0 40px", fontSize: 13, color: "#22c55e", fontWeight: 600 }}>Student pays ₹50 for 5 min · You earn ₹{TOPPER.topperCut}/min (60%)</p>
          <div style={{ display: "flex", gap: 40, alignItems: "center" }}>
            <div style={{ textAlign: "center" }}>
              <button onClick={declineCall} style={{ width: 70, height: 70, borderRadius: "50%", background: "linear-gradient(135deg,#ef4444,#dc2626)", border: "none", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 24px rgba(239,68,68,0.5)", marginBottom: 8 }}><PhoneOff size={28} /></button>
              <p style={{ margin: 0, fontSize: 11, color: "#64748b" }}>Decline</p>
            </div>
            <div style={{ textAlign: "center" }}>
              <button onClick={acceptCall} style={{ width: 70, height: 70, borderRadius: "50%", background: "linear-gradient(135deg,#22c55e,#16a34a)", border: "none", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 24px rgba(34,197,94,0.5)", marginBottom: 8, animation: "callRing 0.8s ease-in-out infinite" }}>
      ...
      {showPayout && (
        <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "flex-end", background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)" }} onClick={() => setShowPayout(false)}>
          <div style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "#13192b", borderRadius: "24px 24px 0 0", borderTop: "1px solid rgba(99,102,241,0.2)", padding: "20px 20px 32px" }} onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => setShowPayout(false)} style={{ width: "100%", padding: 14, borderRadius: 16, background: "linear-gradient(135deg,#6366f1,#8b5cf6)", border: "none", color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", boxShadow: "0 4px 20px rgba(99,102,241,0.4)" }}>🚀 Request Payout</button>
      ...
      {callAccepted && (
        <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "linear-gradient(180deg,#071a10,#0d1117)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
          ...
          <div style={{ width: 96, height: 96, borderRadius: "50%", overflow: "hidden", border: "3px solid #22c55e", animation: "callRing 1.5s ease-in-out infinite" }}>
            <img src={INCOMING_STUDENT.avatar} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "top" }} />
          </div>
          <div style={{ textAlign: "center" }}>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(8px)", display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
          <div style={{ width: "100%", maxWidth: 480, background: "#13192b", borderRadius: "24px 24px 0 0", padding: "24px 20px 36px", borderTop: "1px solid rgba(239,68,68,0.25)" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setShowTopperEndConfirm(false)} style={{ flex: 1, padding: 14, borderRadius: 14, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", color: "#e2e8f0", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Continue</button>
              <button onClick={endTopperCall} style={{ flex: 1, padding: 14, borderRadius: 14, background: "linear-gradient(135deg,#ef4444,#dc2626)", border: "none", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>End Call</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(8px)", display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
          <div style={{ width: "100%", maxWidth: 480, background: "#13192b", borderRadius: "24px 24px 0 0", padding: "24px 20px 36px", borderTop: "1px solid rgba(239,68,68,0.3)" }}>
            {!topperReportConfirm ? (<>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setShowTopperReport(false)} style={{ flex: 1, padding: 14, borderRadius: 14, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", color: "#e2e8f0", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Cancel</button>
                <button onClick={() => setTopperReportConfirm(true)} style={{ flex: 1, padding: 14, borderRadius: 14, background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.4)", color: "#f87171", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Report →</button>
              </div>
            </>) : (<>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => { setTopperReportConfirm(false); setShowTopperReport(false); }} style={{ flex: 1, padding: 14, borderRadius: 14, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", color: "#e2e8f0", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Go Back</button>
                <button onClick={() => { if (callTimerRef.current) clearInterval(callTimerRef.current); setShowTopperReport(false); setTopperReportConfirm(false); setCallAccepted(false); setTimeout(() => setShowTopperRating(true), 400); }} style={{ flex: 1, padding: 14, borderRadius: 14, background: "linear-gradient(135deg,#ef4444,#dc2626)", border: "none", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Submit 🚨</button>
              </div>
            </>)}
          </div>
        </div>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px" }}>
          <div style={{ width: "100%", maxWidth: 420, background: "linear-gradient(180deg,#13192b,#0d1117)", borderRadius: 28, padding: "28px 22px", border: "1px solid rgba(34,197,94,0.2)" }}>
            {!topperRatingDone ? (<>
              ...
              <button onClick={() => setTopperRatingDone(true)} disabled={topperRatingStar === 0} style={{ width: "100%", padding: 14, borderRadius: 16, marginBottom: 10, background: topperRatingStar > 0 ? "linear-gradient(135deg,#22c55e,#16a34a)" : "rgba(255,255,255,0.05)", border: "none", color: topperRatingStar > 0 ? "#fff" : "#334155", fontSize: 14, fontWeight: 800, cursor: topperRatingStar > 0 ? "pointer" : "not-allowed" }}>Submit Rating ✓</button>
              <button onClick={() => setShowTopperRating(false)} style={{ width: "100%", padding: 10, borderRadius: 12, background: "transparent", border: "none", color: "#475569", fontSize: 12, cursor: "pointer" }}>Skip</button>
            </>) : (
